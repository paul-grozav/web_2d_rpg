...
load_textures(gl, config){
  var textures = [];
  for(var i=0; i<config.texture_sprites.length; i++)
  {
    var texture_sprite = config.texture_sprites[i];
    textures.push(this.load_texture(gl, texture_sprite.url));
  }
  return textures;
}
// -------------------------------------------------------------------------- //
...
//  console.log(fsSource);
  ...
  const shaderProgram = this.init_shader_program(gl, vsSource, fsSource);
...
init_buffers(config, data) {
  ...
  for(var i=0; i<tiles.length; i++)
  {
    data.ground = data.ground.concat(
      this.tile_to_vertex_coordinates(tiles[i], config.settings)
    );
  }
  ...
  for(var i=0; i<config.world.tiles.length; i++)
  {
    ...
    const tile_texture_coordinates =
      this.get_sprite_texture_coordinates_by_id(config, tile.texture);
...
// Draw the scene.
//
draw_scene(gl, programInfo, deltaTime, data) {
...
//
init_shader_program(gl, vsSource, fsSource) {
  const vertexShader = this.load_shader(gl, gl.VERTEX_SHADER, vsSource);
  const fragmentShader = this.load_shader(gl, gl.FRAGMENT_SHADER, fsSource);
